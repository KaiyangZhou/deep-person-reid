...
file_pattern = re.compile("^([0-9]+)_([0-9]+)_[0-9]+_[0-9]+_[0-9]+\.jpg")


class MiiMRecorded(ImageDataset):
    ...
    def __init__(self, root="", split_id=0, **kwargs):
        ...
        self.crop_path = os.path.join(self.root, "selected_miim_recorded")
        ...
        super(MiiMRecorded, self).__init__(train, query, gallery, **kwargs)
    ...
    def prepare_split(self, crop_path):
        if not os.path.exists(self.split_path):
            ...
            for file in os.listdir(crop_path):
                ...
                if match:
                    object_guid: int = int(match.group(1))
